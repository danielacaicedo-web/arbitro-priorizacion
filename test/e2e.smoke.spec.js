...
// pruebas separado. El registro que crea queda marcado como responsable
// "ZZ_TEST_AUTOMATIZADO", y las reglas de Firestore permiten borrar SOLO los
// documentos con ese nombre exacto — por eso el test se limpia solo al final
// (bloque finally), pase o falle, y nunca queda visible en el tablero real.
...
test("flujo completo: evaluar, responder, ver informe y guardar en el tablero", async ({ page }) => {
  ...
  try {
    ...
    // S y db son `let`/`const` de nivel superior en el script de la app — no cuelgan
    // de `window`, pero sí son visibles para page.evaluate (misma consola de la página).
    expedienteId = await page.evaluate(() => (typeof S !== "undefined" && S.exp && S.exp.id) || null);
    ...
    // Limpieza: borra el registro de prueba para no dejar nada visible en el tablero real.
    if (expedienteId) {
      await page.evaluate(id => db.collection("expedientes").doc(id).delete(), expedienteId).catch(() => {});
    }
  }
});
